...
describe("App component", () => {
  test("displays the server status message", async () => {
    ...
    const statusElement = await waitFor(() =>
      screen.getByText(/Server Status/i),
    );
  ...
  test("displays that the server is up with a message", async () => {
    render(<App />);
    const statusElement = await waitFor(() =>
      screen.getByText(/Server is up: Service is running/i),
    );
    expect(statusElement).toBeInTheDocument();
  });
  ...
  test("handles server error response", async () => {
    ...
    render(<App />);
    const statusElement = await waitFor(() =>
      screen.getByText(/Server responded with an error./i),
    );
    expect(statusElement).toBeInTheDocument();
  });
  ...
  test("handles server exception", async () => {
    ...
    render(<App />);
    const statusElement = await waitFor(() =>
      screen.getByText(/Server is down or unreachable./i),
    );
    expect(statusElement).toBeInTheDocument();
  });
});
